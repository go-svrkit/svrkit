...
func (c mapCall) apply(m MapInterface[string, int64]) (int64, bool) {
	switch c.op {
	case opLoad:
		return m.Load(c.k)
	case opStore:
	...
	case opCompareAndSwap:
		if m.CompareAndSwap(c.k, c.v, int64(rand.Int())) {
			m.Delete(c.k)
			return c.v, true
		}
		return 0, false
...
func (mapCall) Generate(r *rand.Rand, size int) reflect.Value {
	c := mapCall{op: mapOps[rand.Intn(len(mapOps))], k: randKey(r)}
	switch c.op {
...
func applyCalls(m MapInterface[string, int64], calls []mapCall) (results []mapResult, final map[string]int64) {
	for _, c := range calls {
		v, ok := c.apply(m)
...
func applyMap(calls []mapCall) ([]mapResult, map[string]int64) {
	return applyCalls(new(SyncMap[string, int64]), calls)
}

func applyRWMutexMap(calls []mapCall) ([]mapResult, map[string]int64) {
	return applyCalls(new(RWMutexMap[string, int64]), calls)
}
...
func TestConcurrentRange(t *testing.T) {
	...
	for n := iters; n > 0; n-- {
		...
		if len(seen) != mapSize {
			t.Fatalf("Range visited %v elements of %v-element TreeMap", len(seen), mapSize)
		}
	}
}
...
func TestMapRangeNestedCall(t *testing.T) { // Issue 46399
	...
	m.Range(func(key, value any) bool {
		m.Range(func(key, value any) bool {
			...
			// 42 to the TreeMap. In this case, the key 42 and associated value should be
			// removed from the TreeMap. Therefore any future range won't observe key 42
		...
		// Remove key from TreeMap on-the-fly.
		m.Delete(key)
		return true
	})
	...
	if length != 0 {
		t.Fatalf("Unexpected sync.TreeMap size, got %v want %v", length, 0)
	}
}
