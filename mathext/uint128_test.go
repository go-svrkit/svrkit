// Copyright (c) 2019 Luke Champine, distributed under MIT License
...
func randUint128() Uint128 {
	...
	rand.Read(randBuf)
	return Uint128FromBytes(randBuf)
}
...
func TestUint128(t *testing.T) {
	...
	for i := 0; i < 1000; i++ {
		...
		if Uint128FromBig(x.Big()) != x {
			t.Fatal("Uint128FromBig is not the inverse of Big for", x)
		}

		b := make([]byte, 16)
		x.PutBytes(b)
		if Uint128FromBytes(b) != x {
			t.Fatal("Uint128FromBytes is not the inverse of PutBytes for", x)
		}

		if !x.Equals(x) {
			t.Fatalf("%v does not equal itself", x.Lo)
		}
		if !Uint128From64(x.Lo).Equals64(x.Lo) {
			t.Fatalf("%v does not equal itself", x.Lo)
		}

		if x.CompareTo(y) != x.Big().Cmp(y.Big()) {
			t.Fatalf("mismatch: cmp(%v,%v) should equal %v, got %v", x, y, x.Big().Cmp(y.Big()), x.CompareTo(y))
		} else if x.CompareTo(x) != 0 {
			t.Fatalf("%v does not equal itself", x)
		}

		if x.Compare64(y.Lo) != x.Big().Cmp(Uint128From64(y.Lo).Big()) {
			t.Fatalf("mismatch: cmp64(%v,%v) should equal %v, got %v", x, y.Lo,
				x.Big().Cmp(Uint128From64(y.Lo).Big()), x.Compare64(y.Lo))
		} else if Uint128From64(x.Lo).Compare64(x.Lo) != 0 {
			t.Fatalf("%v does not equal itself", x.Lo)
		}
	}

	// Check Uint128FromBig panics
	...
	checkPanic(func() { _ = Uint128FromBig(big.NewInt(-1)) }, "value cannot be negative")
	checkPanic(func() { _ = Uint128FromBig(new(big.Int).Lsh(big.NewInt(1), 129)) }, "value overflows Uint128")
}
...
func TestArithmetic(t *testing.T) {
	...
	checkBinOp64X := func(x Uint128, op string, y uint64, fn func(x Uint128, y uint64) Uint128, fnb func(z, x, y *big.Int) *big.Int) {
		t.Helper()
		xb, yb := x.Big(), Uint128From64(y).Big()
	...
	for i := 0; i < 1000; i++ {
		...
		if y64 != 0 {
			...
			modfn := func(x Uint128, y uint64) Uint128 {
				return Uint128From64(x.Mod64(y))
			}
...
func TestString(t *testing.T) {
	for i := 0; i < 1000; i++ {
		...
		y, err := Uint128FromString(x.String())
		if err != nil {
			t.Fatal(err)
		} else if !y.Equals(x) {
			t.Fatalf("mismatch:\n%v !=\n%v", x.String(), y.String())
	...
	// Test 0 string
	if Uint128Zero.String() != "0" {
		t.Fatalf(`Zero.String() should be "0", got %q`, Uint128Zero.String())
	}
	// Test Max string
	if Uint128Max.String() != "340282366920938463463374607431768211455" {
		t.Fatalf(`Max.String() should be "0", got %q`, Uint128Max.String())
	}
	// Test parsing invalid strings
	if _, err := Uint128FromString("-1"); err == nil {
...
func BenchmarkDivision(b *testing.B) {
	...
	x64 := Uint128From64(randU64())
	y64 := Uint128From64(randU64())
...
func TestFromBytesBE(t *testing.T) {
	...
	u1 := Uint128FromBytesBE(ip)
	u2, err := Uint128FromString(ips)
	if err != nil {
		t.Fatal(err)
	}
	if u1 != u2 {
